...
// SQL query to fetch all approved bookings
$sql = "SELECT a.id AS appointment_id, CONCAT(u.firstname, ' ', u.lastname) AS full_name, a.appointment_date, a.appointment_time, a.payment_method, a.total_price, a.status, GROUP_CONCAT(s.name SEPARATOR ', ') AS service_names 
        FROM appointments a
    ...
    <div class="container mt-5">
        <div class="card shadow-lg border-0">
            <div class="card-body p-4">
                <h2 class="mb-4 text-center text-warning">Pending Bookings</h2>
                <table id="pendingBookingsTable" class="table table-striped table-hover table-bordered bookingTable" style="width:100%">
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td><a href='../../bws_ui/booking/process/approve_booking.php?id=" . $row['appointment_id'] . "' class='btn btn-success' data-bs-toggle='tooltip' title='Approve'><i class='fas fa-check'></i></a></td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='8' class='text-center'>No pending bookings found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
                ...
                switch (message) {
                    ...
                    case 'error':
                        Swal.fire({
                            ...
                            text: 'There was an error approving the booking. Please try again.',
                        });
                        break;
                    case 'invalid':
                        Swal.fire({
