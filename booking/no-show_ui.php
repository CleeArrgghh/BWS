    ...
    <div class="container mt-5">
        <div class="card shadow-lg border-0">
            <div class="card-body p-4">
                <h2 class="mb-4 text-center text-danger">No-Show Bookings</h2>
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        <?php
                        // SQL query to fetch data from no_show_bookings table
                        $sql_no_show = "SELECT appointment_id, full_name, service_names, appointment_date, appointment_time, total_price FROM no_show_bookings";
                        $result_no_show = $conn->query($sql_no_show);

                        // Check if there are results
                        if ($result_no_show && $result_no_show->num_rows > 0) {
                            // Loop through and output data of each row
                            while ($row_no_show = $result_no_show->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td><strong>" . htmlspecialchars($row_no_show['full_name']) . "</strong></td>";
                                echo "<td>" . htmlspecialchars($row_no_show['service_names']) . "</td>";
                                echo "<td>" . htmlspecialchars($row_no_show['appointment_date']) . "</td>";
                                echo "<td>" . htmlspecialchars($row_no_show['appointment_time']) . "</td>";
                                echo "<td>₱" . number_format($row_no_show['total_price'], 2) . "</td>";
                                echo "<td>";
                                echo "<form method='post' action='../booking/process/no-show_process.php' class='d-inline'>";
                                echo "<input type='hidden' name='appointment_id' value='" . htmlspecialchars($row_no_show['appointment_id']) . "'>";
                                echo "</form>";
                                echo " ";
                                echo "<form method='post' action='../booking/process/no-show_process.php' class='d-inline'>";
                                echo "<input type='hidden' name='appointment_id' value='" . htmlspecialchars($row_no_show['appointment_id']) . "'>";
                            ...
                            echo "<tr><td colspan='6' class='text-center'>No no-show bookings found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
